...
load_informative_priors <- function(file_path, species_name, n_occ_covs, default_sd = sqrt(10)) {
  priors <- get_vague_priors(n_occ_covs, n_det_covs, default_sd = default_sd)
...
for (spp in possible_bats) {
  ...
  priors <- if (PRIOR_TYPE == "informative") {
    ...
    get_vague_priors(n_occ_covs, n_det_covs)
  }
...
saveRDS(stan_data, here(paste0("data/processed/results/stan/stan_data_", PRIOR_TYPE, "_spatial.rds")))

## Design matrices for parameter naming / prediction downstream
saveRDS(
  list(
    xmat_all = xmat_all,
    xmat_cliff = xmat_cliff,
    xmata = xmata,
    xmatb = xmatb,
    xmatc = xmatc,
    xmatd = xmatd,
...
species_to_run <- 1:length(possible_bats)
