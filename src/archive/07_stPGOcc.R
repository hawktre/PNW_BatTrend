renv::activate()
## ---------------------------
## Purpose of script: Prepare detection and covariate data for spatio-temporal
##                    occupancy model using spOccupancy (stPGOcc).
##                    Supports weakly informative and informative priors.
##                    Outputs a species-level list of data objects and
##                    fits the stPGOcc model for each species.
##
...
## Spatial coordinates for sampled grid cells (projected EPSG:5070 in meters)
coords <- covars %>%
...
PRIOR_TYPE  <- "informative" # Options: "weakly_informative", "informative"
...
load_informative_priors_spocc <- function(file_path, species_name, is_cliff, n_det_params = 5, default_var = 10, dist_mat) {
  ...
  # Map parameters: Intercept, Forest (log_fc), Precip, Elevation (dem_max)
  ...
  # Cliff species: add cliff cover (log_cliff) prior
  if (is_cliff) {
    means <- c(means, get_val("Cliffs"))
    sds   <- c(sds, get_val("CliffsSD"))
  }
  
  # Assign to beta.normal (year_eff stays at mean = 0, var = default_var)
...
output_dir <- here("data/processed/results/stPGOcc/full/fits")
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
}

## Bundled spOccupancy data
saveRDS(spocc_data, here(paste0("data/processed/results/stPGOcc/spocc_data_", PRIOR_TYPE, ".rds")))
saveRDS(spocc_data, here(paste0("data/processed/results/stPGOcc/spocc_data_", PRIOR_TYPE, "_spatial.rds")))
...
saveRDS(
  ...
  here("data/processed/results/stPGOcc/index_keys.rds")
)
...
fit_single_species <- function(spp) {
  cat(sprintf("[%s] Starting stPGOcc fit for %s (%s)...\n", format(Sys.time(), "%H:%M:%S"), spp, PRIOR_TYPE))
...
cat("\nAll stPGOcc model fits complete.\n")
