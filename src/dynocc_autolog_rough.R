...
covars_join <- covars %>%
  filter(precip >= 0) |> 
  select(sample_unit_id, grts_id, state, p_forest, precip, dem_mean, topo_rough, contains("samp"), geometry) |> 
  mutate(
    p_forest = log(p_forest + 1),
    across(p_forest:topo_rough, ~ as.vector(scale(.x)))
  ) %>%
  st_drop_geometry() |> 
  clean_names()
...
dets <- dets %>%
  ...
  ungroup() %>%
  drop_na(dem_mean, p_forest, precip, topo_rough) %>%
...
xmat_all <- nw_grid_all %>%
  ...
  select(log_fc, precip, dem_mean, topo_rough) %>%
  as.matrix()
...
# Prior Configuration -----------------------------------------------------
## "informative" or "weakly_informative"
...
psi_regional_summary |> 
  ggplot(aes(x = year, y = mean_regional))+
  geom_point()+
  geom_line()+
